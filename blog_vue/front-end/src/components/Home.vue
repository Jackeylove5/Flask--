    ...
    <div class="card border-0 g-mb-15">
      ...
      <div v-if="posts" class="card-block g-pa-0" >
        <div v-for="(post, index) in posts.items" v-bind:key="index" class="media g-brd-around g-brd-gray-light-v4 g-pa-20 g-mb-20">
          <router-link v-bind:to="{ name: 'User', params: { id: post.author.id }}" v-bind:title="post.author.name || post.author.username">
          ...
          <div class="media-body">
            <div class="d-sm-flex justify-content-sm-between align-items-sm-center g-mb-15 g-mb-10--sm">
              ...
              <div class="text-nowrap g-font-size-12">
                <span>{{ $moment(post.timestamp).fromNow() }}</span> / <router-link v-bind:to="{ name: 'User', params: { id: post.author.id }}"><span v-if="post.author.name">{{ post.author.name }}</span><span v-else>{{ post.author.username }}</span></router-link>
              </div>
            </div>
